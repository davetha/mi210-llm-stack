...
| In-kernel efficiency of `scaled_mm` | **ACTED ON — `docs/43`.** The 44% was Triton launching 40 workgroups on a 104-CU card at M=1. Routing to AITER's CK GEMM is worth **1.48× decode** at TP=2, 1.71× at TP=1 |
| In-kernel efficiency of `fused_moe` | **Open, and now the largest decode kernel** — same question as `docs/43` asked: is a generic fallback running where an AITER kernel exists? |
