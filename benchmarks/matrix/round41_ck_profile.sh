...
echo "=== $(date -u +%T) round 41: rocprofv3 decode decomposition, graph + eager ==="
...
cat > "$LOGS/rd41-aiter-patch.py" <<'EOF'
import pathlib
...
run_profile() {  # $1 = label, $2 = VLLM_ROCM_USE_AITER_LINEAR 0|1
    ...
    echo ""
    echo "=== $(date -u +%T) attempt: $label (AITER_LINEAR=$eager) ==="
    docker run --rm --name "probe-$label" \
      ...
      -v "$LOGS/rd41-aiter-patch.py":/tmp/aiter_patch.py:ro \
      ...
      -e VLLM_ROCM_USE_AITER_LINEAR="$eager" \
      --entrypoint bash vllm-mi210:v0.26.1rc0-ckgemm-warm -c '
set -e
...
run_profile rd41-triton 0
run_profile rd41-ck     1

echo ""
echo "=== $(date -u +%T) round 41 done ==="
...
a, atot = decode_table(f"{L}/rd41-triton")
b, btot = decode_table(f"{L}/rd41-ck")
if not a or not b:
...
print(f"decode-window busy: Triton {atot/1e9:.3f} s, CK {btot/1e9:.3f} s "
      "(8 s window each)")
print()
print(f"{'kernel':<54}{'Triton ms':>11}{'CK ms':>10}{'delta ms':>10}")
print("-" * 85)
keys = set(a) | set(b)
for k in sorted(keys, key=lambda k: -abs(a.get(k, 0) - b.get(k, 0)))[:14]:
    av, bv = a.get(k, 0) / 1e6, b.get(k, 0) / 1e6
    mark = "  <-- NEW in CK" if k not in a else (
        "  <-- GONE in CK" if k not in b else "")
...
for lab in rd41-triton rd41-ck; do
