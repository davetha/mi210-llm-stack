...
echo ""
echo "THE AITER ROWS MEASURE NOTHING. Corrected after the first run: this"
echo "trailer used to say ~1.0x was expected because the harness is"
echo "single-stream. That is a real effect but it is NOT what these rows show."
echo "Both arms selected ROCM_ATTN and loaded ZERO gfx90a code objects --"
echo "_get_backend_priorities() appends ROCM_ATTN unconditionally first, so"
echo "ROCM_AITER_FA is admitted and never chosen unless"
echo "configs/prefer_aiter_fa_gfx90a.py is applied AND VLLM_PREFER_AITER_FA=1."
echo "Confirm with:  grep -c LoadKernel logs/rd32-awq32k-patched.serverlog"
echo "A 0 there means the comparison is ROCM_ATTN vs ROCM_ATTN. Do not report"
echo "it as evidence about AITER either way."
