...
docker run -d --name "$NAME" \
  ...
  -p "${PORT}:8000" \
  `# NCCL_P2P_DISABLE defaults to 0 as of 2026-07-31. It was hardcoded to 1 on` \
  `# the premise that "no xGMI" means "no peer-to-peer", which is false: the` \
  `# driver advertises a PCIe P2P link with NoPeerToPeerDMA clear, a measured` \
  `# device-to-device copy runs at 26.98 GB/s against 14.16 GB/s staged through` \
  `# pinned host memory, and round 31 measures +11.2% prefill / -10.3% TTFT at` \
  `# TP=2 from enabling it. Decode is unchanged (+1.3%, noise). See` \
  `# env/gfx90a-common.env for the full result and the revert instruction.` \
