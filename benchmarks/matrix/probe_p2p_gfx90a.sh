...
for dev in $(lspci -D | grep -iE "Instinct|Processing accel" | cut -d' ' -f1); do
    ...
    lspci -vvs "$up" 2>/dev/null | grep -E "ACSCtl" | sed 's/^/    /' || echo "    (no ACS control line)"
done
...
echo
echo "Measured on this box 2026-07-31: both links PCIe/32000/flags=3 (P2P DMA"
echo "permitted), and ACSCtl shows ReqRedir+ CmpltRedir+ on both upstream"
echo "bridges -- so peer traffic is redirected through the root complex rather"
echo "than routed bridge-to-bridge. That is functional but not the short path."
echo "Since amd_iommu=off, ACS is buying little isolation here while still"
echo "costing the redirect; pcie_acs_override=downstream is a separate lever to"
echo "evaluate only after plain P2P is shown to help."
