...
- [`docs/60-the-expert-cache-question-was-already-answered.md`](docs/60-the-expert-cache-question-was-already-answered.md) — **do not build a MoE expert cache; the trace already ran and lost.** `mi210-vllm/docs/ROUTING.md` (2026-08-06) instrumented the GLM-5.2 router across a six-language mixed workload and replayed it through LRU and static top-K: per-layer entropy **~0.91** (near-uniform over 256 experts), Gini 0.51, top-10% of experts covering only **42%** of accesses, LRU tracking the static curve — and ~2.6 GiB free VRAM per card after the offload window. At 8-of-256 experts per token the cost is the 8 experts × 76 layers crossing PCIe, whichever ones they are. The **MiMo 25/48 CPU split the proposal optimizes no longer exists on this box** — the weights are deleted and production now serves coder/deephat/glm-5.2/laguna with no llama-swap. Also corrects two claims that keep being cited as successes: the vLLM `PrefetchOffloader` **crashed in `ncclAllReduce`** (`docs/29 §2`, round18 is the retry, header reads "WHAT FAILED") and selective UVA was **never measured** (round20 states predictions; the only UVA result is a VM fault at weight load). `guides/moe-expert-cache-vllm.md` corrected in the same change, on both counts, precisely: `--moe-expert-cache-size` is **not in any image here** (open PR #37190) — though *unmerged* is not the objection, this stack carries five unmerged vLLM branches on purpose; the objection is that #37190 needs `--enforce-eager`, which surrenders the 99.9%-coverage graph decode of `docs/42` (eager control: 3.5× slower wall), and is BF16/no-EP>1 so it cannot serve Q4_K at all. And the `VLLM_USE_AITER=0` line is **true of stock AITER, false as a hardware claim** — it is `aiter-cdna2` repatching 242 of 1,422 code objects that makes AITER reachable on gfx90a, not upstream — with the caveat that `ROCM_AITER_FA` **silently corrupts sliding-window attention** on this card
...
- [`guides/moe-expert-cache-vllm.md`](guides/moe-expert-cache-vllm.md) — vLLM expert cache on gfx90a — ⚠️ **partly retracted**, see [`docs/60`](docs/60-the-expert-cache-question-was-already-answered.md): the flag is not shipped and its `VLLM_USE_AITER=0` advice is wrong. MoE-config section still good
