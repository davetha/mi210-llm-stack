...
> **DSV4-Flash prefill campaign (2026-08-25)**: DeepSeek-V4-Flash (284B/13B-active MoE) prefill on llama.cpp — **60K 501 → 545 tok/s, 16K → 601, 100K 451 → 491**, TTFT@60K 120 → 111 s; cumulative with the earlier gather-sparse-attention work, **362 → 545 at 60K (+50.7%)**. Two of the three shipped patches are **bit-identical**, verified against llama.cpp's own reference (`test-backend-ops` 144/144 and 2920/2920): the MFMA lightning-indexer was reloading a 16 KB Q tile once per kv-block, and upstream's MLA K-tile-reuse (`V_is_K_view`) is hardcoded to head-dim 576 while DeepSeek-V4-Flash passes V==K at **512** — the latter is an upstream candidate. Also **corrects a stale claim in this repo's own records**: GPU overlap is not `0.00s` — measured **96.2% / 94.0% busy, 0.6% dual-idle** over 370k dispatches, so pipeline parallelism was already working and the imagined 2× lever does not exist. Eleven further hypotheses were built and killed with measurements, including a tile-dim change that the config contract calls results-neutral but which **silently corrupts output**. See [docs/63](docs/63-dsv4-flash-prefill-campaign.md) and [changes/30](changes/30-dsv4-flash-prefill-wins.md).
