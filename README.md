...
- [`docs/58-cu-masking-on-gfx90a.md`](docs/58-cu-masking-on-gfx90a.md) — **CU masking is real on gfx90a, and it survives graph replay**: `hipExtStreamCreateWithCUMask` reads back identical, 16/52/104-bit masks place work on exactly 16/52/104 physical CUs with **zero overlap** between disjoint sets, and a graph captured on a 16-CU stream **replays on 16 CUs** (the undocumented gate that would have killed the line — vLLM decode is graph-replayed). Two traps for anyone building on it: the flat mask is de-interleaved as **`SE = bit % 8`**, so 13 contiguous bits span all eight shader engines rather than one, and a masked stream takes a **dedicated HSA queue** against a 32-queue budget. But the headline is a negative — two disjoint-masked streams overlapped at **1.99×**, and *two ordinary unmasked streams also reached 1.99×*. HIP already overlaps independent streams; masking buys **isolation, not concurrency**. Probe kernel is pure-ALU spin, so it says nothing yet about HBM contention, and the line must beat `docs/52`'s already-adopted DP=2 (1.118×) while contending for the power budget `docs/53` shows is already binding on prefill
